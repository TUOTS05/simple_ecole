...
class EnrollmentController extends Controller
{
    ...
    //     return view('app.enrollments.index', compact('enrollments', 'schoolYears'));
    // }
    ...
    //         // Action (pour le bouton "Ajouter enfant de mêmes parents")
    ...
    //         $nextNumber = $lastStudent ? (intval(substr($lastStudent->matricule, -4)) + 1) : 1;
    //         $matricule = 'MAT-' . $year . '-' . str_pad($nextNumber, 4, '0', STR_PAD_LEFT);

    //         // 2. Gérer l'upload de la photo de l'élève
    //         $studentPhotoPath = null;
    //         if ($request->hasFile('student_photo')) {
    //             $studentPhotoPath = $request->file('student_photo')->store('students/photos');
    //         }
    ...
    //             'school_year_id' => SchoolYear::where('school_id', $schoolId)->where('is_active', true)->value('id'),
    ...
    //         // 5. Gérer les documents uploadés (optionnel - à adapter selon votre modèle Document)
    //         if ($request->hasFile('documents')) {
    //             foreach ($request->file('documents') as $key => $file) {
    //                 if ($file) {
    //                     $path = $file->store('students/documents');
    //                     // Document::create([
    //                     //     'student_id' => $student->id,
    //                     //     'title' => "Document $key",
    //                     //     'file_path' => $path,
    ...
    //         // 6. Gérer le bouton "Ajouter enfant de mêmes parents"
    //         if ($request->input('action') === 'add_sibling') {
    //         // Sauvegarder les infos des parents en session
    ...
    //         return redirect()->route('app.enrollments.create')
    //             ->with('success', "✅ Enfant enregistré ! Ajoutez maintenant le frère/la sœur.")
    //             ->withInput(); // <-- Très important pour garder les autres champs
    //     }
