...
@section('title', 'Ma Messagerie - Espace Parent')
@section('page_title', 'Messagerie')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    
    <!-- En-tête avec bouton d'action CORRECT pour le parent -->
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
            <h3 class="font-bold text-gray-800">Boîte de réception & Envoyés</h3>
        </div>
                ...
                <a href="{{ route('parent.messages.show', $message->id) }}" 
                   class="block p-5 hover:bg-gray-50 transition duration-200 group {{ !$message->is_read && $message->receiver_id === auth()->id() ? 'bg-blue-50/50' : '' }}">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
                        ...
                        @else
                            @if($message->reply)
                                <span class="flex-shrink-0 bg-green-100 text-green-800 text-xs px-2.5 py-1 rounded-full font-medium">✓ Répondu</span>
                            @else
                                <span class="flex-shrink-0 bg-yellow-100 text-yellow-800 text-xs px-2.5 py-1 rounded-full font-medium">⏳ En attente</span>
                            @endif
                        @endif
                    </div>
