...
@section('title', 'Tarifs Goûter')
@section('page_title', 'Tarifs du Goûter (Maternelle)')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <form method="GET" action="{{ route('gouter.rates.index') }}" class="flex gap-4 items-end">
            <div>
        ...
        <a href="{{ route('gouter.rates.create', ['school_year_id' => $schoolYearId]) }}"
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="text-right py-3 px-4 text-sm font-semibold text-gray-600">Montant Annuel</th>
                    <th class="text-center py-3 px-4 text-sm font-semibold text-gray-600">Modalité</th>
                    <th class="text-center py-3 px-4 text-sm font-semibold text-gray-600">Échéances</th>
                    <th class="text-right py-3 px-4 text-sm font-semibold text-gray-600">Montant / Échéance</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="py-3 px-4 text-center text-gray-600 capitalize">{{ $rate->payment_modality }}</td>
                    <td class="py-3 px-4 text-center text-gray-600">{{ $rate->number_of_installments }}</td>
                    <td class="py-3 px-4 text-right text-gray-800">{{ number_format($rate->installment_amount, 0, ',', ' ') }} FCFA</td>
                    <td class="py-3 px-4 text-center">
                        <a href="{{ route('gouter.rates.edit', $rate->id) }}" class="text-primary hover:text-primary-dark mr-3">✏️ Modifier</a>
                        <form action="{{ route('gouter.rates.destroy', $rate->id) }}" method="POST" class="inline" onsubmit="return confirm('Supprimer ce tarif ?')">
                            @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="6" class="py-12 text-center text-gray-500">
                        Aucun tarif de goûter défini pour cette année scolaire.
                    </td>
                </tr>
                @endforelse
            </tbody>
