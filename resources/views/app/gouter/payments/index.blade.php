...
@section('title', 'Paiements Goûter')
@section('page_title', 'Paiements du Goûter (Maternelle)')

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6" x-data="gouterPaymentForm()">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
        ...
        <form action="{{ route('gouter.payments.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div>
                    ...
                    <select name="gouter_subscription_id" x-model="selectedSubscriptionId" @change="updatePaymentDetails" :disabled="!selectedClassId" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary bg-white disabled:bg-gray-100">
            ...
            <div x-show="selectedSubscription" class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h4 class="font-semibold text-blue-900 mb-2">Échéances concernées par ce paiement</h4>
                        <p class="text-sm text-blue-800 mb-2">Le paiement sera automatiquement réparti sur les échéances les plus anciennes en premier :</p>
                        <div class="flex flex-wrap gap-2">
                            <template x-for="label in selectedSubscription?.unpaid_installments" :key="label">
                                <span class="px-2 py-1 bg-red-100 text-red-700 text-xs font-bold rounded-full" x-text="label"></span>
                            </template>
                            <span x-show="!selectedSubscription?.unpaid_installments || selectedSubscription.unpaid_installments.length === 0" class="text-sm text-gray-500">
                                Aucune échéance impayée.
                            </span>
                        </div>
                    </div>
                    <div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden" x-data="{ showResults: {{ $classId ? 'true' : 'false' }} }">
        <div class="p-6 border-b border-gray-100 bg-gray-50">
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                <h3 class="text-lg font-bold text-gray-800">📜 Historique des Paiements</h3>

                <form method="GET" action="{{ route('gouter.payments.index') }}" class="flex flex-wrap items-end gap-3 w-full lg:w-auto">
                    <input type="hidden" name="school_year_id" value="{{ $schoolYearId }}">
                    ...
                    @if($classId)
                        <a href="{{ route('gouter.payments.index', ['school_year_id' => $schoolYearId]) }}" class="text-sm text-red-600 hover:text-red-800 hover:bg-red-50 px-3 py-2 rounded-lg transition flex items-center gap-1 h-[38px]">
                            ✕ Réinitialiser
                        </a>
                    @endif
                </form>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="py-3 px-4 text-sm text-gray-600">{{ $payment->subscription->gouterRate->schoolClass->name ?? 'N/A' }}</td>
                            ...
                            <td class="py-3 px-4 text-sm text-gray-600">{{ $payment->receivedByUser->name ?? ($payment->receivedByUser->full_name ?? 'Système') }}</td>
                            <td class="py-3 px-4 text-center">
                                <a href="{{ route('gouter.payments.receipt', $payment->id) }}" class="text-primary hover:text-primary-dark text-sm" target="_blank">📥 PDF</a>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="8" class="py-12 text-center text-gray-500">
                                Aucun paiement trouvé pour cette classe.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div x-show="!showResults" class="py-16 text-center">
            ...
            <p class="text-gray-500 text-lg font-medium">Sélectionnez une classe pour afficher l'historique</p>
        </div>
    </div>
</div>
        ...
        return {
            ...
            async fetchSubscriptions() {
                ...
                try {
                    ...
                    const response = await fetch(`/gouter/subscriptions-by-class?class_id=${this.selectedClassId}&school_year_id=${schoolYearId}`);
            ...
            updatePaymentDetails() {
                ...
                this.paymentAmount = this.selectedSubscription ? this.selectedSubscription.remaining : '';
            },
