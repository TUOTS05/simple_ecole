            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                        Numéro Admission
                        <span class="text-xs text-gray-500 font-normal ml-1">(Aperçu)</span>
                    </label>
                    <input type="text" id="admission_number_preview" value="{{ old('admission_number') }}" readonly
                        class="w-full px-4 py-2.5 bg-gray-100 border border-gray-300 text-gray-600 text-sm rounded-lg cursor-not-allowed"
                        placeholder="Généré côté serveur après sauvegarde">
                    <p class="text-xs text-gray-500 mt-1">Numéro d'admission réel généré au serveur</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="matricule_preview" value="{{ old('matricule') }}" readonly
                           class="w-full px-4 py-2.5 bg-gray-100 border border-gray-300 text-gray-600 text-sm rounded-lg cursor-not-allowed"
                           placeholder="Généré côté serveur après sauvegarde">
                    <p class="text-xs text-gray-500 mt-1">Matricule réel généré au serveur</p>
                </div>
                ...
                <div>
                    ...
                    <select name="status" readonly class="w-full px-4 py-2.5 bg-gray-100 border border-gray-300 text-gray-600 text-sm rounded-lg cursor-not-allowed">
                        <option value="inactive" selected>Inactif (En attente)</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <label class="flex items-center">
                            <input type="radio" name="large_family" value="1" {{ old('large_family') == '1' ? 'checked' : '' }} required
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4 mt-2">
                        <label class="flex items-center">
                            <input type="radio" name="staff_child" value="1" {{ old('staff_child') == '1' ? 'checked' : '' }} required
                                   class="w-4 h-4 text-primary border-gray-300 focus:ring-primary">
                            <span class="ml-2 text-sm text-gray-700">OUI</span>
                        </label>
                        <label class="flex items-center">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                        N° Reçu d'inscription
                    </label>
                    <input type="text" name="receipt_number" id="receipt_number" value="{{ old('receipt_number') }}" readonly
                        class="w-full px-4 py-2.5 bg-gray-100 border border-gray-300 text-gray-600 text-sm rounded-lg cursor-not-allowed"
                        placeholder="Généré automatiquement après sauvegarde">
                </div>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <input type="tel" name="guardian_phone" value="{{ old('guardian_phone') }}" required
                           class="w-full px-4 py-2.5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-2 focus:ring-primary focus:border-primary transition @error('guardian_phone') border-red-500 @enderror">
...
<script>
// 1. Logique pour pré-remplir les champs du gardien si père ou mère est sélectionné
// function toggleGuardianFields() {
//     const guardianType = document.querySelector('input[name="guardian_type"]:checked');
//     if (!guardianType) return;
    
//     const guardianName = document.querySelector('input[name="guardian_name"]');
    
//     if (guardianType.value === 'father') {
//         const fatherName = document.querySelector('input[name="father_name"]').value;
//         if (fatherName) guardianName.value = fatherName;
//     } else if (guardianType.value === 'mother') {
...
function toggleGuardianFields() {
    ...
    const guardianFirstName = document.getElementById('guardian_first_name');
    const guardianLastName = document.getElementById('guardian_last_name');
    
    if (guardianType === 'father') {
        const fatherName = document.querySelector('input[name="father_name"]').value;
        if (fatherName) {
            const names = fatherName.split(' ');
            guardianFirstName.value = names[0] || '';
            guardianLastName.value = names.slice(1).join(' ') || '';
        }
    } else if (guardianType === 'mother') {
        const motherName = document.querySelector('input[name="mother_name"]').value;
        if (motherName) {
            const names = motherName.split(' ');
            guardianFirstName.value = names[0] || '';
            guardianLastName.value = names.slice(1).join(' ') || '';
        }
    }
...
// 2. Logique pour l'adresse actuelle
function toggleAddressFields() {
    ...
    if (checkbox && currentAddress && guardianAddress) {
        if (checkbox.checked) {
...
// 3. Logique pour l'adresse permanente
function togglePermanentAddressFields() {
    ...
    if (checkbox && permanentAddress && currentAddress) {
        if (checkbox.checked) {
...
// Les aperçus du numéro d'admission et du matricule sont juste indicatifs.
document.addEventListener('DOMContentLoaded', function() {
    const year = new Date().getFullYear();

    const admissionField = document.getElementById('admission_number_preview');
    if (admissionField && !admissionField.value) {
        const randomNum = Math.floor(Math.random() * 9000) + 1000;
        admissionField.value = `ADM-${year}-${randomNum}`;
    }

    const matriculeField = document.getElementById('matricule_preview');
