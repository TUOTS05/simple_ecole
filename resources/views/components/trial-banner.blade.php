...
$isUrgent = !$isDemo && $days <= 7;
    ...
    <div class="{{ $isUrgent ? 'bg-red-600' : 'bg-gradient-to-r from-blue-600 to-green-500' }} text-white px-4 py-2.5 text-center text-sm font-medium flex items-center justify-center">
    <i class="fas fa-{{ $isDemo ? 'flask' : 'gift' }} mr-2"></i>
    <span>
        @if($isDemo)
        Vous êtes en mode <strong>Démonstration</strong>.
        @else
    ...
    <a href="{{ route('request-account') }}" target="_blank"
        class="ml-3 bg-white {{ $isUrgent ? 'text-red-600' : 'text-blue-600' }} px-3 py-1 rounded-full font-bold hover:bg-gray-100 transition text-xs">
